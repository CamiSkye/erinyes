        ...
        {isQuizOpen && (
          <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-2xl w-full h-full max-w-[90vw] max-h-[90vh] relative">
              <button className="absolute top-4 right-4 cursor-pointer" onClick={closeQuiz}>
                ❌
              </button>
                ...
                >
                    ...
                    <div className="flex items-start justify-between w-full text-left pr-4">
                      <div className="flex items-start space-x-4 flex-1">
                        ...
                        <div className="flex-1 min-w-0">
                          <h3 className="text-lg font-semibold text-[#8B5E3C] mb-1">
                            {t(`training.courses.${courseKey}.title`)}
                          </h3>
                          <p className="text-sm text-[#6B6B6B] mb-3">
                            {t(`training.courses.${courseKey}.description`)}
                          </p>
                          <div className="flex flex-wrap gap-2 mb-3">
                            <Badge className={getCategoryColor(course.category)}>
                              {t(`training.categories.${course.category}`)}
                            </Badge>
                            <Badge className={getLevelColor(course.level)}>
                              {t(`training.levels.${course.level}`)}
                            </Badge>
                          </div>
                         <div className="flex flex-col sm:flex-row gap-3 text-sm text-[#6B6B6B]">
                            ...
                            <div className="text-[#9B7FD7] italic sm:ml-auto">
                              {t("training.languageNotice")}
                            </div>
                          </div>
                        </div>
                      </div>
                  ...
                  <AccordionContent className="px-6 pb-6">
                    <div className="pt-4 border-t border-[#E5E1DA]">
                      <h4 className="font-semibold text-[#8B5E3C] mb-4">
                        {t("training.modulesLabel")}
                      </h4>
                          ...
                          <div key={module.id} className="flex items-start gap-3 p-3 bg-[#F5F1EA] rounded-lg hover:bg-[#E5E1DA]/50 transition-colors">
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center justify-between mb-1">
                                <h5 className="font-medium text-[#8B5E3C]">
                                  {courseModules[moduleIndex]?.title}
                                </h5>
                              ...
                              <p className="text-sm text-[#6B6B6B]">
                                {courseModules[moduleIndex]?.description}
                              </p>
                            </div>
                          </div>
                        ))}
                      ...
                      <Button
                        ...
                        className="bg-[#9B7FD7] hover:bg-[#8B6BC7] text-white flex-1 cursor-pointer w-full"
                      >
                        <PlayCircle className="mr-2 h-4 w-4" />
                        {t("training.startFormation")}
                      </Button>
                    </div>
                  </AccordionContent>
                </AccordionItem>
